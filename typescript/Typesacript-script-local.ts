...
function main() {
  data.forEach((element, index) => {
    const astronaut: number[][] = element.astronaut;
    const n: number = element.n;
    const outputExpected: number = element.outputExpected;
    console.log(`#!#!#! ::: ------------------------------------------`);
    console.log(`#!#!#! ::: Test #${index + 1}`);
    console.log(`#!#!#! ::: Lista de astronautas: ${JSON.stringify(astronaut)}`);
    console.log(`#!#!#! ::: n: ${n}`);
    const startTime = performance.now();
    const result: number = journeyToMoon(n, astronaut);
    const endTime = performance.now();
    console.log(`#!#!#! ::: Tiempo de ejecucion ${endTime - startTime} milliseconds`);
    console.log(`#!#!#! ::: Salida esperada: ${outputExpected}`);
  ...
  console.log(`#!#!#! ::: Astronaut list: ${JSON.stringify(data2.astronaut)}`);
  console.log(`#!#!#! ::: n (IDs): ${data2.n}`);
  console.log(`#!#!#! ::: Number of executions: ${max}`);
  for (let index = 0; index < max; index++) {
    const astronaut = new Array(...data2.astronaut);
    const startTime = performance.now();;
    const result: number = journeyToMoon(data2.n, astronaut);
    const endTime = performance.now();
    totalTime += (endTime - startTime);
    // console.log(`#!#!#! ::: Run #${index} -> Expected: ${data2.outputExpected}, Output: ${result}`);
  }
  console.log(`#!#!#! ::: Tiempo promedio de ejecucion: ${(totalTime/max)/1000} seg.`);
}
